...
def main():
    ...
    (
        ...
        .add_metadata(Loader(url, pull_requests))
        .filter_operator("pullRequests > 0")
        .add_metadata(Loader(url, commits))
        .filter_operator("commits > 20")
        .add_metadata(Loader(url, weeks_of_activity))
        .filter_operator("weeksOfActivity >= 50")
        .add_metadata(Loader(url, issues))
        .filter_operator("issues > 10")
        .add_metadata(Loader(url, contributors))
        .filter_operator("contributors >= 10")
        .add_metadata(Loader(url, defective_commits))
        .filter_operator("defective_commits >= 10")
